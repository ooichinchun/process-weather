...
import shutil
#sudo python3 count2tbb.py -d 1 -sd 2018-07-28-07:00 -ed 2018-07-29-06:00
# -------------------------------------------------------
...
def convert_tbb(chn, req_path, desdir, para):
    if chn in ['TIR', 'SIR']:
        ...
        with open(desdir+'grid05.dat', 'wb') as outfile:
            for dat in dats:
                with open(dat, 'rb') as infile:
                    for line in infile:
                        outfile.write(line)
...
def main(req_path,save_path,sdate,edate,tstep,chn,num,compiler,debug):
    ...
    source    = '/gridded/FD/V20151105/'  # old data path: V20151105
    ...
    for file in tqdm(files, desc='total progress'):
        ...
        tmp_check = ntpath.basename(file)[0:8]
        filename    = ntpath.basename(file)
        ...
        # cmd_list = ['dd', 'if=F:/Ubuntu/WeatherPrediction/FinalProjectHandover/Source/HimawariData/data/2019/11/201911190700.tir.01.fld.geoss', 'of=F:/Ubuntu/WeatherPrediction/FinalProjectHandover/Source/HimawariData/data/little.geoss', 'conv=swab']
        call (['dd', 'if='+destination[:-4], 'of='+desdir+'little.geoss',
                    'conv=swab'], stderr=subprocess.PIPE, stdout=subprocess.PIPE)
        ...
        nc_file  = (filename.rsplit('.',3)[0]).replace('.', '_') + chn + str(num) + '.nc'
        ...
        cp = subprocess.run(command, stderr=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
        ...
        print(filename,desdir," ... Chec this file")
